...
defmodule PastryNode do
  ...
  def getRandomBonusNode(totalNodes, bonusparam, nodes) do
    if bonusparam > 0 do
        bname = PastryNode.generate_node_name(:rand.uniform(totalNodes))
  ...
  defp message_forward(messages, total_nodes, destination, hops) do
    ...
    if Enum.member?(Enum.at(messages, 6), next_hop) == true do
    ...
    map = visited_nodes |> Enum.reduce(%{}, fn x, acc -> Map.update(acc, x, 1, &(&1 + 1)) end)
    occurence = map[next_hop] 
    if occurence == nil do
      occurence = 0
    end
  ...
  def actor_generation(index, total_nodes, routing_table, failure) do
      if index <= total_nodes do
          ...
          {:ok, pid} = GenServer.start_link(PastryNode, [index, 1, leaf_minus, leaf_plus, route_table, [], failure] , name: new_node_name)
          :global.register_name(new_node_name,pid)
          node_arrival_message(index-1, node_id)
          actor_generation(index+1, total_nodes, routing_table, failure)
      end
  end
...
def main(args) do
  ...
  failure_nodes = Enum.at(args,2) |> String.to_integer
  rows = 32
  ...
  failure = PastryNode.getRandomBonusNode(num_nodes, failure_nodes, [])
  failure = Enum.uniq(failure)
  PastryNode.actor_generation(1, num_nodes, routing_table, failure)
